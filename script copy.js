...
// global variables:
// let map, mapEvent;
// class App with methods - blueprint( guide for making something)

class Workout {
  // as we create a new object we want to creat a date of it immediately
  // new Date() - creates a Date object representing the current date/time
  date = new Date();
  // creating a unique ID
  // timestamp -Date.now() - returns the number of milliseconds since midnight 01 January, 1970 UTC
  ...
  _setDescription() {
    // to tel prettier extention in our code to not to put all line by line, it should ignore the next line
    //  prettier-ignore
    const months = [
      'January',
      'February',
    ...
    // this.date.getMonth() gives us the number from 0 - 11, so months its an array that 0 index will give us January
...
class App {
  ...
  _getPosition() {
    if (navigator.geolocation)
      // this._load map as a callback will by called by js and pass in the position argument as soon as the user position is determined
      // When I pass a method an object is to another function as a callback, the this is lost. this is undefined. So I  use the bind method to bind _loadMap to the getposition object -Using bind() to borrow methods from a different object
      // The bind() method creates a new function, when invoked, has the this sets to a provided value.
      // The bind() method allows an object to borrow a method from another object without making a copy of that method: https://www.javascripttutorial.net/javascript-bind/
      // bind(this) points to the current object
      // loadMap is treated as the regular function call, not as a method call - since it is a calback dunction we are not calling ot ourselves, its getCurrentPosition function  who is callking when its gets the current position of the user. when i t calls  _loadMap it calls this method, as a regular function,in in a regular function the this keyword is set to undefined
